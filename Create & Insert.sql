...
VALUES ('Alime','255 Jandex Street, District 13',0901392331,'ChienHugo111@gmail.com'),
    ('Binary','1080 Pixel Street, District 144',0800281220,'DuySocket222@yahoo.com'),
    ('Canary','720 HDMI Street, District 6',0833281520,'LokLok322@hcmut.edu.vn'),
    ('Dharma','480 Low Res Street, District 64',091221120,'ZuDuYu52@icloud.com');
ALTER TABLE CHINHANH
MODIFY COLUMN MaChiNhanh varchar(50);
UPDATE CHINHANH 
SET 
    MaChiNhanh = CONCAT('CN', MaChiNhanh);
-- @block 2 --
CREATE TABLE IF NOT EXISTS HINHANH_CHINHANH (
    HA_MCN VARCHAR(50),
    HinhAnh VARCHAR(255) NOT NULL,
    CONSTRAINT PK_HINHANH PRIMARY KEY (HinhAnh),
    CONSTRAINT FK_HA_MCN FOREIGN KEY (HA_MCN)
...
CREATE TABLE IF NOT EXISTS KHU (
    ...
    CONSTRAINT FK_Khu_MCN FOREIGN KEY (Khu_MCN)
        REFERENCES CHINHANH (MaChiNhanh)
        ON DELETE CASCADE ON UPDATE CASCADE
);
-- @block 3 KHU --
...
CREATE TABLE IF NOT EXISTS LOAIPHONG (
    ...
    SoKhach INT NOT NULL CHECK (SoKhach BETWEEN 1 AND 10),
...
VALUES ('PHONG 1', 10.0, 1, 'PHONG CHO NGUOI CO DON'),
    ('PHONG 2',30.5,5,'PHONG CHO 5 AE TRONG 1 CHIEC XE TANK'),
...
CREATE TABLE IF NOT EXISTS THONGTINGIUONG (
    ...
    SoLuong INT NOT NULL DEFAULT 1 CHECK (SoLuong BETWEEN 1 AND 10),
    CONSTRAINT PK_TTG PRIMARY KEY (KichThuoc),
    CONSTRAINT FK_TTG_MLP FOREIGN KEY (TTG_MLP)
...
CREATE TABLE IF NOT EXISTS CHINHANH_CO_LOAIPHONG (
    ...
    CONSTRAINT FK_Co_MLP FOREIGN KEY (Co_MLP)
        REFERENCES LOAIPHONG (MaLoaiPhong)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_Co_MCN FOREIGN KEY (Co_MCN)
        REFERENCES CHINHANH (MaChiNhanh)
        ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS PHONG (
    ...
    CONSTRAINT FK_Phong_MCN FOREIGN KEY (Phong_MCN)
        REFERENCES KHU (Khu_MCN)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_Phong_TK FOREIGN KEY (Phong_TK)
        REFERENCES KHU (TenKhu)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_Phong_MLP FOREIGN KEY (Phong_MLP)
        REFERENCES LOAIPHONG (MaLoaiPhong)
        ON DELETE CASCADE ON UPDATE CASCADE,
...
UPDATE LOAIVATTU 
SET 
    MaLoaiVatTu = CONCAT('VT', MaLoaiVatTu);
-- @block 9 --
CREATE TABLE IF NOT EXISTS LOAIVATTU_TRONG_LOAIPHONG (
    Trong_MLVT VARCHAR(6),
    Trong_MLP INT,
    SoLuong INT NOT NULL DEFAULT 1 CHECK (SoLuong BETWEEN 1 AND 20),
    CONSTRAINT FK_Trong_MLVT FOREIGN KEY (Trong_MLVT)
        REFERENCES LOAIVATTU (MaLoaiVatTu)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_Trong_MLP FOREIGN KEY (Trong_MLP)
        REFERENCES LOAIPHONG (MaLoaiPhong)
        ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS VATTU (
    ...
    VT_SP VARCHAR(3),
    CONSTRAINT FK_VT_MLVT FOREIGN KEY (VT_MLVT)
        REFERENCES LOAIVATTU (MaLoaiVatTu)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_VT_MCN_SP FOREIGN KEY (VT_MCN , VT_SP)
        REFERENCES PHONG (Phong_MCN , SoPhong)
        ON DELETE CASCADE ON UPDATE CASCADE,
...
UPDATE NHACUNGCAP 
SET 
    MaNhaCungCap = CONCAT('NCC', MaNhaCungCap);
-- @block 12 --
CREATE TABLE IF NOT EXISTS CUNGCAPVATTU (
    CCVT_MNCC VARCHAR(7),
    CCVT_MLVT VARCHAR(6),
    CCVT_MCN VARCHAR(50),
    CONSTRAINT FK_CCVT_MNCC FOREIGN KEY (CCVT_MNCC)
        REFERENCES NHACUNGCAP (MaNhaCungCap)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_CCVT_MLVT FOREIGN KEY (CCVT_MLVT)
        REFERENCES LOAIVATTU (MaLoaiVatTu)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_CCVT_MCN FOREIGN KEY (CCVT_MCN)
        REFERENCES CHINHANH (MaChiNhanh)
        ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS GOIDICHVU (
    TenGoi VARCHAR(50),
    SoNgay INT NOT NULL CHECK (SoNgay BETWEEN 1 AND 100),
    SoKhach INT NOT NULL CHECK (SoKhach BETWEEN 1 AND 10),
...
CREATE TABLE IF NOT EXISTS HOADONGOIDICHVU (
    ...
    CONSTRAINT FK_HDGDV_MKH FOREIGN KEY (HDGDV_MKH)
        ...
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_HDGDV_TG FOREIGN KEY (HDGDV_TG)
...
CREATE TABLE IF NOT EXISTS DONDATPHONG (
    ...
    TinhTrang INT NOT NULL CHECK (TinhTrang BETWEEN 0 AND 3),
    TongTien INT NOT NULL DEFAULT 0 CHECK (TongTien > - 1),
    DDP_MKH VARCHAR(8),
    DDP_TG VARCHAR(50),
    SoKhach INT NOT NULL CHECK (SoKhach > - 1),
    CONSTRAINT FK_DDP_MKH FOREIGN KEY (DDP_MKH)
        REFERENCES KHACHHANG (MaKhachHang)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_DDP_TG FOREIGN KEY (DDP_TG)
        REFERENCES GOIDICHVU (TenGoi)
        ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS PHONGTHUE (
    ...
    CONSTRAINT FK_PT_MDP FOREIGN KEY (PT_MDP)
        REFERENCES DONDATPHONG (MaDatPhong)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_PT_MCN FOREIGN KEY (PT_MCN)
        REFERENCES PHONG (Phong_MCN)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_PT_SP FOREIGN KEY (PT_SP)
...
CREATE TABLE IF NOT EXISTS HOADON (
    ...
    HD_MDP VARCHAR(16),
    CONSTRAINT FK_HD_MDP FOREIGN KEY (HD_MDP)
        REFERENCES DONDATPHONG (MaDatPhong)
        ON DELETE CASCADE ON UPDATE CASCADE,
...
UPDATE HOADON 
SET 
    MaHoaDon = CONCAT('HD', '28112022', MaHoaDon);
-- @block 19 --
...
UPDATE DOANHNGHIEP 
SET 
    MaDoanhNghiep = CONCAT('DN', MaDoanhNghiep);
-- @block 20 --
CREATE TABLE IF NOT EXISTS DICHVU (
    ...
    DV_MDN VARCHAR(6),
    CONSTRAINT FK_DV_MDN FOREIGN KEY (DV_MDN)
...
CREATE TABLE IF NOT EXISTS DICHVUSPA (
    ...
    CONSTRAINT FK_DVS_MDV FOREIGN KEY (DVS_MDV)
...
CREATE TABLE IF NOT EXISTS LOAIHANGDOLUUNIEM (
    ...
    CONSTRAINT FK_LHDLN_MDV FOREIGN KEY (LHDLN_MDV)
        REFERENCES DICHVU (MaDichVu)
        ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS THUONGHIEUDOLUUNIEM (
    ...
    CONSTRAINT FK_THDLN_MDV FOREIGN KEY (THDLN_MDV)
        REFERENCES DICHVU (MaDichVu)
        ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS MATBANG (
    ...
    STTMatBang INT NOT NULL UNIQUE DEFAULT 1 CHECK (STTMatBang BETWEEN 1 AND 50),
    ...
    Logo VARCHAR(255),
    CONSTRAINT FK_MB_MDV FOREIGN KEY (MB_MDV)
        REFERENCES DICHVU (MaDichVu)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_MB_MCN FOREIGN KEY (MB_MCN)
        REFERENCES CHINHANH (MaChiNhanh)
        ON DELETE CASCADE ON UPDATE CASCADE,
...
VALUES ('CN1',1,5,10,1000,'NULL','DVM001','NON SON','A.COM'),
    ...
    ('CN4',4,20,25,4000,'NULL','DVM004','OZ','D.COM');
-- @block 25 --
CREATE TABLE IF NOT EXISTS HINHANHCUAHANG (
    HACH_MCN VARCHAR(50) NOT NULL,
    HACH_STTMatBang INT NOT NULL DEFAULT 1,
    HinhAnh VARCHAR(255),
    CONSTRAINT FK_HACH_MCN FOREIGN KEY (HACH_MCN)
        REFERENCES MATBANG (MB_MCN)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_HACK_STTMatBang FOREIGN KEY (HACH_STTMatBang)
        REFERENCES MATBANG (STTMatBang)
        ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS KHUNGGIOHOATDONG (
    ...
    GioBatDau TIME,
    GioKetThuc TIME,
    CONSTRAINT FK_KGHD_MCN FOREIGN KEY (KGHD_MCN)
        REFERENCES MATBANG (MB_MCN)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FK_KGHD_STTMatBang FOREIGN KEY (KGHD_STTMatBang)
        REFERENCES MATBANG (STTMatBang)
        ON DELETE CASCADE ON UPDATE CASCADE,
